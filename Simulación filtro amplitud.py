...
plt.imshow(np.abs(fftconvolve(np.flipud(np.fliplr(matriz_B)), fftconvolve(matriz_A, np.flipud(np.fliplr(matriz_A)) , mode = "same"), mode = "same")), cmap='gray')
plt.colorbar()
plt.show()
